...
from PySide6.QtCore import QEasingCurve, QPropertyAnimation, QSequentialAnimationGroup, QTimer, Signal, Qt, QSize
...
class MainWindowView(QWidget):
    ...
    def _create_left_menu(self) -> QWidget:
        ...
        self._apply_icon(self.toggle_menu_button, "icon_menu.png", QSize(24, 24))
        layout.addWidget(self.toggle_menu_button)

        def build_menu_button(name: str, text: str, icon_name: str | None = None, register: bool = True) -> QPushButton:
            button = QPushButton(text)
            button.setObjectName(f"menuButton_{name}")
            button.setCursor(Qt.CursorShape.PointingHandCursor)
            button.setCheckable(True)
            if icon_name:
                self._apply_icon(button, icon_name)
            layout.addWidget(button)
            if register:
                self._menu_buttons[name] = button
            return button

        build_menu_button("home", "Home", "cil-home.png")
        build_menu_button("widgets", "Widgets", "cil-view-module.png")
        build_menu_button("create", "Create", "cil-pencil.png")

        self.save_button = build_menu_button("save", "Save", "cil-save.png", register=False)
        ...
        self._apply_icon(self.settings_button, "cil-settings.png")
    ...
    def _apply_icon(self, button: QPushButton, icon_name: str, size: QSize | None = None) -> None:
        """为按钮应用图标，如果资源存在"""
        icon_path = self._icons_dir / icon_name
        if icon_path.exists():
            button.setIcon(QIcon(str(icon_path)))
            button.setIconSize(size or QSize(20, 20))
